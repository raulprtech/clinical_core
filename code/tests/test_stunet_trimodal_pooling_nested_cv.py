...
from evaluate_stunet_trimodal_pooling_nested_cv import (  # noqa: E402
    repeat_metrics,
    select_bimodal_weights,
)
...
class STUNetTrimodalPoolingTests(unittest.TestCase):
    ...
    def test_bimodal_weights_use_train_only_grid(self):
        risks = np.asarray(
            [[0.9, 0.1], [0.7, 0.2], [0.2, 0.8], [0.1, 0.9]],
            dtype=np.float64,
        )
        survival = np.asarray([5.0, 10.0, 20.0, 30.0])
        events = np.asarray([1, 1, 1, 1])
        weights, score = select_bimodal_weights(
            risks, survival, events, step=0.1
        )
                ...
                rows.append(
                    {
                        ...
                        "risk_fusion_bimodal_moments": base + 0.07,
                    }
                )
